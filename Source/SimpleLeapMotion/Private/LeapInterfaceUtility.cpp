...
FVector AdjustForHMD(FVector In)
{
	if (GEngine->XRSystem.IsValid())
	{
		...
		FVector Position;
		OrientationQuat = GEngine->XRSystem->GetBaseOrientation();
		Position = GEngine->XRSystem->GetBasePosition();
		//GEngine->XRSystem->GetCurrentOrientationAndPosition(OrientationQuat, Position);
...
ELeapBasicDirection LeapBasicVectorDirection(FVector Direction)
{
	...
	{
		if (Direction.X < -0.5)
			return ELeapBasicDirection::DIRECTION_TOWARD;
		else if (Direction.X > 0.5)
			return ELeapBasicDirection::DIRECTION_AWAY;
	}
	//is basic in y?
	else if (y >= x && y >= z)
	{
		if (Direction.Y < -0.5)
			return ELeapBasicDirection::DIRECTION_LEFT;
		else if (Direction.Y > 0.5)
			return ELeapBasicDirection::DIRECTION_RIGHT;
	}
	//is basic in z?
	else if (z >= x && z >= y)
	{
		if (Direction.Z < -0.5)
			return ELeapBasicDirection::DIRECTION_DOWN;
		else if (Direction.Z > 0.5)
			return ELeapBasicDirection::DIRECTION_UP;
	}
	
	//If we haven't returned by now, the direction is none
	return ELeapBasicDirection::DIRECTION_NONE;
}
