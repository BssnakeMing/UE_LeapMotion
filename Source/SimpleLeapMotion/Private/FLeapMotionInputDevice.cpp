...
LeapHMDSnapshot HMDSnapshotSamples::CurrentHMDSample()
{
	...
	{
		Snapshot.Orientation = GEngine->XRSystem->GetBaseOrientation();
		Snapshot.Position = GEngine->XRSystem->GetBasePosition();
	}
	return Snapshot;
}
...
void FLeapMotionInputDevice::ParseEvents()
{
	...
	if (GEngine->XRSystem.IsValid() && ControllerData.bTimeWarpEnabled) {
	...
	if (PastState->HandCount != HandCount)
	{
		switch (HandCount)
		{
			case 0:
				...
				EmitAnalogInputEventForKey(EKeysLeap::LeapLeftPalmPitch, 0.f, 0, false);
				EmitAnalogInputEventForKey(EKeysLeap::LeapLeftPalmYaw, 0.f, 0, false);
				EmitAnalogInputEventForKey(EKeysLeap::LeapLeftPalmRoll, 0.f, 0, false);
				EmitAnalogInputEventForKey(EKeysLeap::LeapRightPalmPitch, 0.f, 0, false);
				EmitAnalogInputEventForKey(EKeysLeap::LeapRightPalmYaw, 0.f, 0, false);
				EmitAnalogInputEventForKey(EKeysLeap::LeapRightPalmRoll, 0.f, 0, false);
				...
				{
					ILeapEventInterface::Execute_LeapHandRemoved(EventDelegate, ELeapHandType::HAND_UNKNOWN, nullptr);
				});
				break;
			case 1:
				{
					Leap::Hand Hand = Frame.hands()[0];
					if (Hand.isLeft() && RightHand)
					{
						RightHand->IsValid = false;
						CallFunctionOnDelegates([&](UObject* EventDelegate)
						{
							ILeapEventInterface::Execute_LeapHandRemoved(EventDelegate, ELeapHandType::HAND_RIGHT, RightHand);
						});
					}
					else if (Hand.isRight() && LeftHand)
					{
						LeftHand->IsValid = false;
						CallFunctionOnDelegates([&](UObject* EventDelegate)
						{
							ILeapEventInterface::Execute_LeapHandRemoved(EventDelegate, ELeapHandType::HAND_LEFT, LeftHand);
						});
					}
				}
				break;
	...
	for (int i = 0; i < HandCount; i++)
	{
		...
		if (Hand.isRight())
		{
			if (!RightHand)
			{
				RightHand = NewObject<ULeapHand>();
				RightHand->AddToRoot();
			}
			RightHand->SetHand(Hand);
			RightHand->IsValid = true;
			CallFunctionOnDelegates([&](UObject* EventDelegate)
			{
				ILeapEventInterface::Execute_LeapRightHandMoved(EventDelegate, RightHand);
			});

			//Input Mapping
			FRotator PalmOrientation = PEventHand->PalmOrientation;
			EmitAnalogInputEventForKey(EKeysLeap::LeapRightPalmPitch, PalmOrientation.Pitch * LEAP_IM_SCALE, 0, false);
			EmitAnalogInputEventForKey(EKeysLeap::LeapRightPalmYaw, PalmOrientation.Yaw * LEAP_IM_SCALE, 0, false);
			EmitAnalogInputEventForKey(EKeysLeap::LeapRightPalmRoll, PalmOrientation.Roll * LEAP_IM_SCALE, 0, false);
		}
		else if (Hand.isLeft())
		{
			if (!LeftHand)
			{
				LeftHand = NewObject<ULeapHand>();
				LeftHand->AddToRoot();
			}
			LeftHand->SetHand(Hand);
			LeftHand->IsValid = true;
			CallFunctionOnDelegates([&](UObject* EventDelegate)
			{
				ILeapEventInterface::Execute_LeapLeftHandMoved(EventDelegate, LeftHand);
			});

			//Input Mapping
			FRotator PalmOrientation = PEventHand->PalmOrientation;
			EmitAnalogInputEventForKey(EKeysLeap::LeapLeftPalmPitch, PalmOrientation.Pitch * LEAP_IM_SCALE, 0, false);
			EmitAnalogInputEventForKey(EKeysLeap::LeapLeftPalmYaw, PalmOrientation.Yaw * LEAP_IM_SCALE, 0, false);
			EmitAnalogInputEventForKey(EKeysLeap::LeapLeftPalmRoll, PalmOrientation.Roll * LEAP_IM_SCALE, 0, false);
		}
		...
		if (Grabbed && !PastHandState.Grabbed)
		{
			...
			//input mapping
			if (PEventHand->HandType == ELeapHandType::HAND_LEFT)
			{
		...
		else if (!Grabbed && PastHandState.Grabbed)
		{
			...
			if (PEventHand->HandType == ELeapHandType::HAND_LEFT)
			{
				EmitKeyUpEventForKey(EKeysLeap::LeapLeftGrab, 0, false);
			}
			else
			{
				EmitKeyUpEventForKey(EKeysLeap::LeapRightGrab, 0, false);
			}
		}
		...
		else
		{
			...
			else if (!Pinched && PastHandState.Pinched)
			{
				...
				if (PEventHand->HandType == ELeapHandType::HAND_LEFT)
				{
					EmitKeyUpEventForKey(EKeysLeap::LeapLeftPinch, 0, false);
				}
				else
				{
					EmitKeyUpEventForKey(EKeysLeap::LeapRightPinch, 0, false);
				}
			}
		}
...
FLeapMotionInputDevice::~FLeapMotionInputDevice()
{
	...
	ClearReferences(LeftHand);
	ClearReferences(RightHand);
	LeftHand = nullptr;
	RightHand = nullptr;
...
void FLeapMotionInputDevice::AddEventDelegate(UObject* EventDelegate)
{
	if (EventDelegate->GetClass()->ImplementsInterface(ULeapEventInterface::StaticClass()))
		EventDelegates.AddUnique(EventDelegate);
