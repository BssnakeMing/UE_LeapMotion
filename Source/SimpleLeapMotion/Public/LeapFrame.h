...
* Frame 类表示在单个帧中检测到的一组手和手指跟踪数据。
* Leap Motion 软件在跟踪区域内检测手、手指和工具，并以 Leap Motion 帧速率在帧中报告它们的位置、方向、手势和运动。
*
* Leap API reference: https://developer.leapmotion.com/documentation/cpp/api/Leap.Frame.html
*/
UCLASS(BlueprintType)
class SIMPLELEAPMOTION_API ULeapFrame : public UObject
{
	GENERATED_BODY()
public:
	ULeapFrame(const FObjectInitializer &ObjectInitializer);
	~ULeapFrame();
